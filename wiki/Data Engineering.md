---
type: overview
tags: [data-engineering]
created: 2026-06-30
updated: 2026-07-08
sources: []
---

# Data Engineering

Page de synthèse pour tout ce qui concerne le data engineering : pipelines, orchestration, modélisation de données, qualité de données, outils (Spark, Airflow, dbt, etc.).

## Pages liées

- [[Ontologie de données]] — modèle métier formel (concepts, propriétés, relations) au-dessus des données brutes.
- [[Code — DQ pandas]] / [[Code — DQ PySpark]] — scripts de qualité de données.
